...
void MainWindow::openInit() {
	...
	if (dialog->exec() == QDialog::Accepted) {
		...
		if (isPureSimulation)
			connect(showDataAct, SIGNAL(triggered()), wp,
					SLOT(showDataWidget()));
		else
...
void MainWindow::createActions() {
	...
	showDataAct = new QAction(QIcon("images/icons/TDataIcon.png"),
			tr("Show &Data"), this);
	showDataAct->setStatusTip(
			tr("Switch to the Data & Statistic of the current simulation"));

	saveAct = new QAction(QIcon("images/icons/TSaveIcon.png"), tr("&Save"),
			this);
	...
	sendAct = new QAction(QIcon("images/icons/TSendIcon.png"), tr("Send"),
			this);
	sendAct->setStatusTip("Send the current simulation to a friend");

	quitAct = new QAction(QIcon("images/icons/TQuitIcon.png"), tr("&Quit"),
			this);
...
void MainWindow::notImplemented() {
	QMessageBox::information(this, tr("Empty Slot"),
			tr("This Slot appears to be empty. \n"
					"Implementation will be added later."), QMessageBox::Ok);
}
